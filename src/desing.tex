...
\subsection{Diseño de anotación}
...
se requiere: (a)anotaciones a determinadas clases de la API de Android y
(b)anotación del aplicativo a evaluar frente a esa política de
seguridad.\newline
Previamente en \ref{subsec:consVerPol}, se definieron los canales que muestran
información con nivel de seguridad bajo y la información considerada con nivel
de seguridad alto. Ahora, para detectar flujos desde información considera con
nivel de seguridad alto hacia canales con nivel de seguridad bajo, se propone el
siguiente esquema de anotación:\newline
Primero, debe existir un top principal al que se le atribuya la autoridad
máxima.\newline
Segundo, en el caso de las variables que almacenan información considerada con
nivel de seguridad alto, el label de seguridad debe indicar que pertenecen al
top principal.\newline
Tercero, en el caso de los canales considerados con nivel de seguridad bajo, los
AL de los argumentos que muestran información que sólo debería ser conocida por
el top Principal, se anota con label público, asegurando que la información sólo se
mostrará si el AL de la invocación del método está asociado con un AL público,
es decir si se envía información considerada como pública.
